...
public class ImageToEmblem1_02 extends PApplet {
...
public void LoadEmblemImage(File _selectedImage) {
  ...
  if (_selectedImage == null){
    ...
    try {
      ...
      String[] splitPathName = split(inputFileName, '/'); // Unix split path
      splitPathName = split(inputFileName, '\\');         // Windows spilt path
      ...
      splitPathName = split(inputFileName, '.'); // remove image extension
      
      inputFileName = splitPathName[0];

    } catch (Exception e){
      fileWasLoaded = false;
      println("An exception occured in: void LoadEmblemImage(), try statement");
    }
  }
}
class GUI{
...
class TimeStamp {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "ImageToEmblem1_02" };
    if (passedArgs != null) {
